...
function LinkEmbedder({ links, updateGame }) {
  ...
  const handleRemoveEmbeddedLink = (url) => {
    updateGame({
      embeddedLinks: embeddedLinks.filter(
        (embeddedLink) => embeddedLink !== url,
      ),
    });
    setEmbeddedLinks(
      embeddedLinks.filter((embeddedLink) => embeddedLink !== url),
    );
  };
        ...
        <div className="ml-2">
          {linkToRemove === url ? (
            <div className="flex gap-1">
              <Button
                className="bg-gray-300 hover:bg-gray-400 text-white rounded-md text-xs"
                onClick={() => setLinkToRemove('')}
              >
                X
              </Button>
              <Button
                className="p-2 bg-red-500 hover:bg-red-600 text-white rounded-md text-xs"
                onClick={() => handleRemoveEmbeddedLink(url)}
              >
                Remove
              </Button>
            </div>
            ...
            <div className="flex ">
              <Button
                className="bg-gray-300 hover:bg-gray-400 text-white rounded-md"
                onClick={() => setLinkToRemove(url)}
              >
                -
              </Button>
            </div>
  ...
  return (
    <div className="p-1 mx-auto max-w-lg w-full">
      ...
      <div className="flex items-center justify-start gap-2 mr-11">
        ...
        />
        <Button
          onClick={() => handleAddEmbeddedLink(inputUrl)}
          className="ml-1 text-white/90  bg-purple-500/50  hover:bg-purple-500/80  rounded-md"
        >
          +
        </Button>
      </div>
      {inputUrlError && (
        <span className="text-red-500 text-xs ml-3 mt-1">{inputUrlError}</span>
      )}
    </div>
  );
}
